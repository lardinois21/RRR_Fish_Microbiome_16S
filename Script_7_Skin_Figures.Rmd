...
Most recent edit: 
- 2/13/26 - LLL - re-running PCoA multi-panel and adding PERMANOVA results to plot
- 4/7/24 - LLL - messing with temp graph & colored axes of PCoA plots
...
## JUMP TO HERE FOR PUBLICATION MAPS!!!
...
pa_map +
  ...
  tm_scalebar(breaks = c(0, 50, 100), text.size = 0.5, position = c("left", "bottom")) #add scale bar
```
...
pa_coiba_map <- pa_coiba_map +
  tm_scalebar(breaks = c(0, 10, 20), text.size = 0.75, position = c("right", "bottom")) + #add scale bar
    tm_shape(TEP_sampling_sites.sf) + #add circles for GPS coords of fish sampling sites
  tm_symbols(fill = "#CC5500", size = 1, shape = 21)+
  tm_layout(frame = TRUE, frame.lwd = 3, frame.color = "#CC5500")
...
pa_lasperlas_map <- pa_lasperlas_map +
  tm_scalebar(breaks = c(0, 10, 20), text.size = 0.75, position = c("right", "bottom"), width = 500) + #add scale bar
  tm_shape(TEP_sampling_sites.sf) + #add circles for GPS coords of fish sampling sites
  tm_symbols(fill = "#009999", size = 1, shape = 21)+
  tm_layout(frame = TRUE, frame.lwd = 3, frame.color = "#009999")

pa_lasperlas_map
```

```{r save inset maps, echo = FALSE}
setwd('./Intermediate_R_objects/Figs')

## save an image ("plot" mode)
tmap_save(pa_lasperlas_map, filename = "Las Perlas Map Inset w GPS Coords w colored box.pdf")

## save an image ("plot" mode)
tmap_save(pa_coiba_map, filename = "Coiba Map Inset w GPS Coords w colored box.pdf")
```
...
tm_compass(type = "8star", size = 3, position = c("right", "top")) + #add compass to map
  tm_scalebar(breaks = c(0, 50, 100), text.size = 0.5, position = c("right", "bottom")) #add scale bar
...
#A. concolor
ROHR_06_Aconcolor <- readRDS("./Intermediate_R_objects/ROHR_06_A_concolor.rds")

#A. troschelii
ROHR_06_Atroschelii <- readRDS("./Intermediate_R_objects/ROHR_06_A_troschelii.rds")

#A. xanthopterus
ROHR_06_Axanthopterus <- readRDS("./Intermediate_R_objects/ROHR_06_A_xanthopterus.rds")

#C. colonus
ROHR_06_Ccolonus <- readRDS("./Intermediate_R_objects/ROHR_06_C_colonus.rds")

#C. panamensis
ROHR_06_Cpanamensis <- readRDS("./Intermediate_R_objects/ROHR_06_C_panamensis.rds")

#C. humeralis
ROHR_06_Chumeralis <- readRDS("./Intermediate_R_objects/ROHR_06_C_humeralis.rds")

#E. labriformis
ROHR_06_Elabriformis <- readRDS("./Intermediate_R_objects/ROHR_06_E_labriformis.rds")

#J. nigrirostris
ROHR_06_Jnigrirostris <- readRDS("./Intermediate_R_objects/ROHR_06_J_nigrirostris.rds")

#M. dorsalis
ROHR_06_Mdorsalis <- readRDS("./Intermediate_R_objects/ROHR_06_M_dorsalis.rds")

#P. laticlavius
ROHR_06_Platiclavius <- readRDS("./Intermediate_R_objects/ROHR_06_P_laticlavius.rds")
```
...
plotlegend <- plot_ordination(
  ...
  scale_color_manual(values = c2,
                     labels = c("Wet", "Dry")) +
  geom_point(aes(color = season), alpha = 0.6, size = 8) +
  geom_point(color = "grey90", size = 1) +
  labs(shape = "Region", color = "Season") +
  theme_bw(base_size = 14) +   # increases all text globally
  theme(
...
#### test adding PERMANOVA results to upper RH corner of plots

## IN PROGRESS 2-13-26 - LLL

```{r add PERMANOVA results to plot above}
#make list of p-vals (rounded to 0.001) and r2 vals (rounded to 2 decimals) from species-by-species PERMANOVAS (see supp mat table S7) - using the results on the unrarefied dataset w/ Bray-Curtis dissimilarity
stats_list <- list(
  A = list(region_p = 0.00539946, region_r2 = 0.044480298, #Aconc2S
           season_p = 0.00039996, season_r2 = 0.057418612), 
  B = list(region_p = 0.00109989, region_r2 = 0.054858075, #Mdor2S
           season_p = 0.00089991, season_r2 = 0.056269142),
  C = list(region_p = 0.0159984,  region_r2 = 0.052707625, #Axan2S
           season_p = 0.055794421, season_r2 = 0.044829817),
  D = list(region_p = 0.00059994, region_r2 = 0.066797361, #Plat2S
           season_p = 0.00239976, season_r2 = 0.058879833),
  E = list(region_p = 0.128387161, region_r2 = 0.034941033, #Jnig2S
           season_p = 0.092590741, season_r2 = 0.036081443),
  'F' = list(region_p = 0.119488051,  region_r2 = 0.042552709, #Chum2S
           season_p = 0.120387961, season_r2 = 0.043454864),
  G = list(region_p = 0.00039996, region_r2 = 0.056363303, #Cpan2S
           season_p = 0.00459954, season_r2 = 0.047179285),
  H = list(region_p = 0.02059794, region_r2 = 0.045115829, #Elab2S
           season_p = 0.02559744, season_r2 = 0.04361031),
  I = list(region_p = 0.00959904,  region_r2 = 0.049376738, #Ccol2S
           season_p = 0.03019698, season_r2 = 0.045760956),
  J = list(region_p = 0.077292271, region_r2 = 0.037059773, #Atro2S
           season_p = 0.717428257, season_r2 = 0.026303451),
  K = list(region_p = 0.00479952, region_r2 = 0.190487786, #Water2S
           season_p = 0.00009999, season_r2 = 0.312923507)
)

#add stats annotation function
add_stats_annotation <- function(p, stats, format_pvals = TRUE) {

  # format p-vals to show only 3 sig figs
  format_p <- function(x) {
    if (format_pvals) {
  ...
  label_text <- sprintf(
    "Region p: %s\n R²: %.2f\nSeason p: %s\n R²: %.2f",
    format_p(stats$region_p),
    stats$region_r2,
    format_p(stats$season_p),
    stats$season_r2
  )
...
#add stats to plots
Aconc_stat <- add_stats_annotation(Aconc2S, stats_list$A)
Mdor_stat <- add_stats_annotation(Mdor2S, stats_list$B)
Axan_stat <- add_stats_annotation(Axan2S, stats_list$C)
Plat_stat <- add_stats_annotation(Plat2S, stats_list$D)
Jnig_stat <- add_stats_annotation(Jnig2S, stats_list$E)
Chum_stat <- add_stats_annotation(Chum2S, stats_list$'F')
Cpan_stat <- add_stats_annotation(Cpan2S, stats_list$G)
Elab_stat <- add_stats_annotation(Elab2S, stats_list$H)
Ccol_stat <- add_stats_annotation(Ccol2S, stats_list$I)
Atro_stat <- add_stats_annotation(Atro2S, stats_list$J)
Water_stat <- add_stats_annotation(Water2S, stats_list$K)

#cowplot together
multi_PCoA_troph_STATS <- plot_grid(Aconc_stat, Mdor_stat, Axan_stat, Plat_stat, Jnig_stat, Chum_stat, Cpan_stat, Elab_stat, Ccol_stat, Atro_stat, Water_stat, legendAC,
          labels=c("A","B","C","D","E","F","G","H","I","J","K"),
          label_size = 15)

multi_PCoA_troph_STATS
```

```{r add fish icons to this plot}
#add icons to graph
multi_PCoA_troph_STATS2 <- ggdraw() +
  draw_plot(multi_PCoA_troph_STATS) +
  draw_image(image = icon_Aconc2, x = -0.430, y =  0.460, scale = 0.060) +# Panel A
  draw_image(image = icon_Mdor2, x = -0.180, y = 0.460, scale = 0.060) +# Panel B
  draw_image(image = icon_Axan2, x =  0.070, y =  0.460, scale = 0.060) +# Panel C
  draw_image(image = icon_Plat2, x = 0.320, y = 0.460, scale = 0.060) +# Panel D
...
multi_PCoA_troph_STATS2
```

```{r code for exporting plot above as a png}
ggsave(filename = "Fish Skin PCoA Plots 21-22 - color coded by trophic group w STATS.png", plot = multi_PCoA_troph_STATS2, path = './Intermediate_R_objects/Figs', width = 16, height = 10, device='png', dpi=700)
```

```{r code for exporting plot above as a pdf}
ggsave(filename = "Fish Skin PCoA Plots 21-22 - color coded by trophic group w STATS.pdf", plot = multi_PCoA_troph_STATS2, path = './Intermediate_R_objects/Figs', width = 16, height = 10, device='pdf', dpi=600)
```
...
#A. concolor
ROHR_06_Aconcolor.rar <- readRDS("./Intermediate_R_objects/ROHR_06_A_concolor_rar.rds")

#A. troschelii
ROHR_06_Atroschelii.rar <- readRDS("./Intermediate_R_objects/ROHR_06_A_troschelii_rar.rds")

#A. xanthopterus
ROHR_06_Axanthopterus.rar <- readRDS("./Intermediate_R_objects/ROHR_06_A_xanthopterus_rar.rds")

#C. colonus
ROHR_06_Ccolonus.rar <- readRDS("./Intermediate_R_objects/ROHR_06_P_colonus_rar.rds")

#C. panamensis
ROHR_06_Cpanamensis.rar <- readRDS("./Intermediate_R_objects/ROHR_06_C_panamensis_rar.rds")

#C. humeralis
ROHR_06_Chumeralis.rar <- readRDS("./Intermediate_R_objects/ROHR_06_C_humeralis_rar.rds")

#E. labriformis
ROHR_06_Elabriformis.rar <- readRDS("./Intermediate_R_objects/ROHR_06_E_labriformis_rar.rds")

#J. nigrirostris
ROHR_06_Jnigrirostris.rar <- readRDS("./Intermediate_R_objects/ROHR_06_J_nigrirostris_rar.rds")

#M. dorsalis
ROHR_06_Mdorsalis.rar <- readRDS("./Intermediate_R_objects/ROHR_06_M_dorsalis_rar.rds")

#P. laticlavius
ROHR_06_Platiclavius.rar <- readRDS("./Intermediate_R_objects/ROHR_06_P_laticlavius_rar.rds")
...
ROHR_08_obj_clean_updated_P <- readRDS("./Intermediate_R_objects/ROHR_08_obj_clean_updated_phyla.rds")
