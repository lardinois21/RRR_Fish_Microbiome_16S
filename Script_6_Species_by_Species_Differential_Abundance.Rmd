...
Feb 15-18, 2026 --> (LLL) expanding analysis of DA taxa to investigate ecological importance
...
We're starting with the "ROHR06_new" dataset, which was previously cleaned a bit in the Water differential abundance script (W-Script_3...) to remove extra metadata columns and change characters that R doesn't like (e.g., spaces between Las & Perlas). 

```{r read in cleaned fish skin dataset from water DA script}
#from script 4.1
ROHR06_new = readRDS("./Intermediate_R_objects/ROHR_06_obj_newMetadata_cleaned.rds")
```
...
# Store original taxa names (sequences / OTU IDs)
original_taxa_names <- taxa_names(ROHR06_new)
...
# Create key table BEFORE overwriting taxa_names (to be able to link taxa back to sequences)
taxa_key_table <- data.frame(
  original_taxa_name = original_taxa_names,
  new_taxa_name = new_taxa_names,
...
saveRDS(ROHR06_new, "./Intermediate_R_objects/ROHR_06_obj_newTaxanames.rds")
```

```{r also save key table as csv}
write_csv(taxa_key_table, file = "./Intermediate_R_objects/ROHR_06_obj_newTaxanames_KEY.csv")
```

```{r read new names if not starting from top}
#read if not starting from top
ROHR06_new = readRDS("./Intermediate_R_objects/ROHR_06_obj_newTaxanames.rds")
```
...
#A. concolor
saveRDS(ROHR_06_Aconcolor, "./Intermediate_R_objects/ROHR06_newA_concolor.rds")

#M. dorsalis
saveRDS(ROHR_06_Mdorsalis, "./Intermediate_R_objects/ROHR06_newM_dorsalis.rds")

#A. xanthopterus
saveRDS(ROHR_06_Axanthopterus, "./Intermediate_R_objects/ROHR06_newA_xanthopterus.rds")

#P. laticlavius
saveRDS(ROHR_06_Platiclavius, "./Intermediate_R_objects/ROHR06_newP_laticlavius.rds")

#C. colonus
saveRDS(ROHR_06_Ccolonus, "./Intermediate_R_objects/ROHR06_newP_colonus.rds")

#C. panamensis
saveRDS(ROHR_06_Cpanamensis, "./Intermediate_R_objects/ROHR06_newC_panamensis.rds")

#E. labriformis
saveRDS(ROHR_06_Elabriformis, "./Intermediate_R_objects/ROHR06_newE_labriformis.rds")

```
#Read subsets
```{r read subsets}
ROHR_06_Aconcolor = readRDS("./Intermediate_R_objects/ROHR06_newA_concolor.rds")

ROHR_06_Mdorsalis = readRDS("./Intermediate_R_objects/ROHR06_newM_dorsalis.rds")

ROHR_06_Axanthopterus = readRDS("./Intermediate_R_objects/ROHR06_newA_xanthopterus.rds")

ROHR_06_Platiclavius = readRDS( "./Intermediate_R_objects/ROHR06_newP_laticlavius.rds")

ROHR_06_Ccolonus = readRDS( "./Intermediate_R_objects/ROHR06_newP_colonus.rds")

ROHR_06_Cpanamensis = readRDS( "./Intermediate_R_objects/ROHR06_newC_panamensis.rds")

ROHR_06_Elabriformis = readRDS( "./Intermediate_R_objects/ROHR06_newE_labriformis.rds")
```
...
#Save phyloseq object with filtered taxa
saveRDS(ROHR_06_Aconcolor_filt, "./Intermediate_R_objects/ROHR_06_Aconcolor_filt_10percent_newTaxanames.rds")
saveRDS(ROHR_06_Mdorsalis_filt, "./Intermediate_R_objects/ROHR_06_Mdorsalis_filt_10percent_newTaxanames.rds")
saveRDS(ROHR_06_Axanthopterus_filt, "./Intermediate_R_objects/ROHR_06_Axanthopterus_filt_10percent_newTaxanames.rds")
saveRDS(ROHR_06_Platiclavius_filt, "./Intermediate_R_objects/ROHR_06_Platiclavius_filt_10percent_newTaxanames.rds")
saveRDS(ROHR_06_Cpanamensis_filt, "./Intermediate_R_objects/ROHR_06_Cpanamensis_filt_10percent_newTaxanames.rds")
saveRDS(ROHR_06_Elabriformis_filt, "./Intermediate_R_objects/ROHR_06_Elabriformis_filt_10percent_newTaxanames.rds")
saveRDS(ROHR_06_Ccolonus_filt, "./Intermediate_R_objects/ROHR_06_Ccolonus_filt_10percent_newTaxanames.rds")
```

```{r read filtered 10% taxa}
#read if not starting from top
ROHR_06_Aconcolor_filt = readRDS("./Intermediate_R_objects/ROHR_06_Aconcolor_filt_10percent_newTaxanames.rds")
ROHR_06_Mdorsalis_filt = readRDS("./Intermediate_R_objects/ROHR_06_Mdorsalis_filt_10percent_newTaxanames.rds")
ROHR_06_Axanthopterus_filt = readRDS("./Intermediate_R_objects/ROHR_06_Axanthopterus_filt_10percent_newTaxanames.rds")
ROHR_06_Platiclavius_filt = readRDS("./Intermediate_R_objects/ROHR_06_Platiclavius_filt_10percent_newTaxanames.rds")
ROHR_06_Cpanamensis_filt = readRDS("./Intermediate_R_objects/ROHR_06_Cpanamensis_filt_10percent_newTaxanames.rds")
ROHR_06_Elabriformis_filt = readRDS("./Intermediate_R_objects/ROHR_06_Elabriformis_filt_10percent_newTaxanames.rds")
ROHR_06_Ccolonus_filt = readRDS("./Intermediate_R_objects/ROHR_06_Ccolonus_filt_10percent_newTaxanames.rds")
```
...
summary(resOrdered) #Las Perlas non-upwelling vs. upwelling
summary(resOrderedLC_n) #Las Perlas non-upwelling vs. Coiba non-upwelling
summary(resOrderedLC_nU) #Las Perlas non-upwelling vs. Coiba upwelling
summary(resOrderedLC_up) #Las Perlas upwelling vs. Coiba upwelling
summary(resOrderedCC) #Coiba non-upwelling vs. upwelling
summary(resOrderedLC_Un) #Las Perlas upwelling vs. Coiba non-upwelling
...
```{r checking padj <0.05 in our comparisons}
#NOTE: LLL corrected these 2-16-26 (not sure why the commented in values were wrong when the plot was right)
sum(resOrdered$padj < 0.05, na.rm=TRUE) #7
sum(resOrderedLC_n$padj < 0.05, na.rm=TRUE) #4
sum(resOrderedLC_nU$padj < 0.05, na.rm=TRUE) #3
sum(resOrderedLC_up$padj < 0.05, na.rm=TRUE) #5
sum(resOrderedCC$padj < 0.05, na.rm=TRUE) #1
sum(resOrderedLC_Un$padj < 0.05, na.rm=TRUE) #12
```
...
### Extract ALL fish DA sequences

Right now, we have info on the taxa that are differentially abundant across all our fish, but not very detailed info on what sorts of microbe they are. Furthermore, since we will want to compare the fish and water DA taxa downstream, we want to be able to identify the unique ASVs linked to each of these taxa, then compare ASVs (since we generally have assignations to genus level and above, two different microbial taxa could be counted as the same if we go by taxonomic assignment). To do this, we'll read in the taxa key table we created earlier when we transformed the original phyloseq object to append taxonomy rather than sequences as rownames.

#### read in fish sequences to query NCBI with
```{r read in sequence key from above}
taxa_key_table <- read_csv(file = "./Intermediate_R_objects/ROHR_06_obj_newTaxanames_KEY.csv")
```

```{r define list of species}
top_results_list <- list(
  A_concolor  = top_resultsAcon,
  M_dorsalis = top_resultsMdor,
  A_xanthopterus = top_resultsAxan,
  P_laticlavius = top_resultsPlat,
  C_panamensis = top_resultsCpan,
  E_labriformis = top_resultsElab,
  C_colonus = top_resultsCcol
)
```

```{r fix marinimicrobia non-matching name issue}
top_results_list <- imap(top_results_list, function(df, name) {
  
  df %>%
    mutate(
      Species = if_else(
        str_detect(Species, "^Marinimicrobia"),
        paste0("Marinimicrobia (SAR406 clade); ", Species),
        Species
      )
    )
})
...
```{r pull and combine all DA taxa and seqs}
DA_seqs_list <- imap(top_results_list, function(top_df, host_name) {

  # ---- STEP 1: Collapse duplicate taxa entries (Species) ----
  top_collapsed <- top_df %>%
    mutate(
      contrast_log2FC = paste(contrast, log2FoldChange)
    ) %>%
    group_by(Species) %>%
    mutate(n_occur = n()) %>%
    summarise(
      contrast_log2FC = paste(contrast_log2FC, collapse = " ; "),
      n_occur = first(n_occur),
      .groups = "drop"
    )

  # ---- STEP 2: First-pass exact match between taxa key and top DA taxa per fish species ----
  first_pass <- taxa_key_table %>%
    left_join(
      top_collapsed,
      by = c("new_taxa_name" = "Species")
    )

  # ---- STEP 3: Keep matched only ----
  fish_DA <- first_pass %>%
    filter(!is.na(contrast_log2FC)) %>%
    mutate(host_species = host_name) %>%
    select(
      host_species,
      new_taxa_name,
      n_occur,
...
```{r validate DA seqs}
#check that the number of recovered DA seqs matches the original number of significant DA taxa
validation_counts <- imap_dfr(DA_seqs_list, function(df, host_name) {
  
  tibble(
    host_species = host_name,
    recovered_total = sum(df$n_occur, na.rm = TRUE),
    original_total  = nrow(top_results_list[[host_name]]),
    match = recovered_total == original_total
  )
})

validation_counts #YAY! this worked!!!
```

```{r bind all fish DA taxa into one table}
FishDA_seqs_all <- bind_rows(DA_seqs_list)
#view all fish DA seqs
FishDA_seqs_all
```

```{r save Fish DA taxa table}
write_csv(FishDA_seqs_all, file = "./Intermediate_R_objects/ALL_Fish_DAtaxa_seqs.csv")
```

## Start here if already run fish DA analyses
```{r read Fish DA taxa table}
FishDA_seqs_all <- read_csv("./Intermediate_R_objects/ALL_Fish_DAtaxa_seqs.csv")
```

### Merge fish and water DA seqs (significant only)

```{r read in water sequence key}
taxa_key_table_W <- read_csv(file = "./Intermediate_R_objects/ROHR_08_obj_newTaxanames_KEY_WATER.csv")
```

```{r read in water DA taxa list}
WaterDA_seqs <- read_csv(file = "./Intermediate_R_objects/WaterDAsequences_DESeq2_20260216.csv")
```

```{r merge water and fish DA tables}
ALL_DA_seqs <- bind_rows(FishDA_seqs_all, WaterDA_seqs)
```

```{r save MERGED DA taxa table}
write_csv(ALL_DA_seqs, file = "./Intermediate_R_objects/MERGED_FishANDWater_DAtaxa_seqs.csv")
```

```{r read MERGED DA taxa table}
ALL_DA_seqs <- read_csv("./Intermediate_R_objects/MERGED_FishANDWater_DAtaxa_seqs.csv")
```

```{r look at unique sequences}
n_distinct(FishDA_seqs_all$original_taxa_name) #129 unique significant DA ASVs across the whole fish dataset

n_distinct(WaterDA_seqs$original_taxa_name) #732 unique significant DA ASVs across the water dataset

n_distinct(ALL_DA_seqs$original_taxa_name) #806 unique significant DA ASVs across the MERGED fish + water dataset (so at least 74 sequences are only DA for the fish dataset) -- note that this does not factor in region-season comparisons
```
...
From the tests run above, we can already tell that the fish skin microbiome stays much more constant between regions and seasons, as there are over 50 times more DA taxa between LP wet & dry for the water compared to the avg. DA in our 7 target fish. Furthermore, it is clear that quite a few ASVs occur across multiple comparisons (either multiple fish, or multiple region-season comparisons). 
...
```{r expand table again to have rows for each contrast}
ALL_DA_expanded <- ALL_DA_seqs %>%
  
  # Expand multiple contrasts into separate rows
  separate_rows(contrast_log2FC, sep = " ; ") %>%
  
  # Extract numeric log2FC (last numeric token)
  mutate(
    log2FoldChange = as.numeric(str_extract(contrast_log2FC, "[-0-9\\.eE]+$")),
    contrast = str_remove(contrast_log2FC, " [-0-9\\.eE]+$")
  ) %>%
  
  select(-contrast_log2FC)

ALL_DA_expanded #1713 entries (WAY more than our 806 unique ASVs)
```

```{r adjust taxonomic names to make comparable across datasets}
#pull names w/o numeric identifiers
ALL_DA_expanded <- ALL_DA_expanded %>%
  mutate(
    new_taxa_base = str_remove(new_taxa_name, "\\.\\d+$")
  )

#check where multiple ASVs have same name
taxon_sequence_check <- ALL_DA_expanded %>%
  group_by(new_taxa_base) %>%
  summarise(
    n_unique_sequences = n_distinct(original_taxa_name),
    .groups = "drop"
  )

#reassign standardized names
ALL_DA_expanded <- ALL_DA_expanded %>%
  group_by(new_taxa_base) %>%
  mutate(
    n_unique_sequences = n_distinct(original_taxa_name)
  ) %>%
  arrange(new_taxa_base, original_taxa_name) %>%
  mutate(
    seq_id = if_else(
      n_unique_sequences > 1,
      dense_rank(original_taxa_name),
      1L
    ),
    new_taxa_name_standardized = if_else(
      n_unique_sequences > 1,
      paste0(new_taxa_base, ".", seq_id),
      new_taxa_base
    )
  ) %>%
  ungroup() %>%
  select(-n_unique_sequences, -seq_id)

#keep only relevant columns
ALL_DA_expanded <- ALL_DA_expanded %>%
  mutate(new_taxa_name = new_taxa_name_standardized) %>%
  select(-new_taxa_name_standardized, -new_taxa_base)

#rename columns to be more informative
ALL_DA_expanded2 <- ALL_DA_expanded %>%
  rename(
    ASV_sequence = original_taxa_name,
    Taxonomy = new_taxa_name
  )
```

```{r save full DA table with harmonized taxa names}
write_csv(ALL_DA_expanded2, file = "./Intermediate_R_objects/MERGED_FishANDWater_DAtaxa_seqs_NEWNAMES.csv")
```

```{r read MERGED DA taxa table}
ALL_DA_expanded2 <- read_csv("./Intermediate_R_objects/MERGED_FishANDWater_DAtaxa_seqs_NEWNAMES.csv")
```


```{r split out water and fish to compare direction of change}
fish_df <- ALL_DA_expanded %>%
  filter(host_species != "Water")

water_df <- ALL_DA_expanded %>%
  filter(host_species == "Water") %>%
  select(
    contrast,
    original_taxa_name,
    water_log2FC = log2FoldChange
  )
```

```{r compare shared taxa}
shared_fish_water <- fish_df %>%
  inner_join(
    water_df,
    by = c("contrast", "original_taxa_name")
  ) %>%
  select(
    contrast,
    new_taxa_name,
    original_taxa_name,
    fish_species = host_species,
    fish_log2FC = log2FoldChange,
    water_log2FC
  )

#add test of direction of change (i.e. are they either enriched or depleted in the same season)
shared_fish_water <- shared_fish_water %>%
  mutate(
    same_direction = sign(fish_log2FC) == sign(water_log2FC)
  )

shared_fish_water

#rename columns for export
shared_fish_water2 <- shared_fish_water %>%
  rename(
    ASV_sequence = original_taxa_name,
    Taxonomy = new_taxa_name,
    host_species = fish_species
  ) %>% 
  select(host_species, Taxonomy, ASV_sequence, everything())
```

```{r save shared fish and water DA taxa table}
write_csv(shared_fish_water2, file = "./Intermediate_R_objects/shared_fish_water_significantDAtaxa_20260217.csv")
```

```{r count total number of shared taxa by species and season-region pairing}
shared_summary <- shared_fish_water %>%
  group_by(fish_species, contrast) %>%
  summarise(
    n_shared = n_distinct(original_taxa_name),
    n_same_direction = sum(sign(fish_log2FC) == sign(water_log2FC)),
    .groups = "drop"
  )
...
```{r count number of unique shared ASVs}
#count number of unique shared ASVs
n_distinct(shared_fish_water$original_taxa_name) #47

#count number of repeated ASVs (shared multiple times)
repeated_ASVs <- shared_fish_water %>%
  count(original_taxa_name) %>%
  filter(n > 1)
nrow(repeated_ASVs) #20

#look into repeated ASVs
asv_vector <- repeated_ASVs$original_taxa_name
shared_repeated_ASVs <- ALL_DA_expanded %>%
  filter(original_taxa_name %in% asv_vector)
shared_repeated_ASVs
```
...
#### Plotting relationships between shared DA taxa in fish & water

```{r filter for fish with at least 4 shared ASVs to calc stats on}
# Keep only panels with >=4 shared ASVs
shared_fish_water_filtered <- shared_fish_water %>%
  group_by(fish_species, contrast) %>%
  filter(n_distinct(original_taxa_name) >= 4) %>%
  ungroup()

# Compute correlation per fish × contrast
cor_stats <- shared_fish_water_filtered %>%
  group_by(fish_species, contrast) %>%
  summarise(
    r = round(cor(water_log2FC, fish_log2FC), 2),
    p_val = signif(cor.test(water_log2FC, fish_log2FC)$p.value, 3),
    n_shared = n_distinct(original_taxa_name),  # fix: count unique ASVs per panel
    .groups = "drop"
  ) %>%
  mutate(
    label = paste0("n=", n_shared,
                   "\nr=", r,
                   "\np=", p_val)
  )

plot_df <- shared_fish_water_filtered %>%
  left_join(cor_stats, by = c("fish_species", "contrast"))
```

```{r plot log2fold change of water against fish samples}
Water_Fish_DAplot1 <- ggplot(plot_df, aes(x = water_log2FC, y = fish_log2FC)) +
  geom_point(alpha = 0.6) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  
  # Add correlation annotation in top-left corner of each facet
  geom_text(
    data = cor_stats,
    aes(
      x = min(shared_fish_water_filtered$water_log2FC),
      y = max(shared_fish_water_filtered$fish_log2FC),
      label = label
    ),
    inherit.aes = FALSE,
    hjust = 0,
  ...
  facet_grid(fish_species ~ contrast, scales = "free") +
  labs(
    title = "", #can add in: Fish vs Water Differential Abundance Effect Sizes
    x = "Water log2FoldChange",
    y = "Fish log2FoldChange"
  ) +
  theme_bw() +
  theme(
...
```{r save plot as PDF}
ggsave(filename = "Log2Fold Change in Significant DA microbes WATER vs FISH.pdf", plot = Water_Fish_DAplot1, path = "./Intermediate_R_objects/Figs", width = 12, height = 8, device='pdf', dpi=600)
```

```{r save plot as PNG}
ggsave(filename = "Log2Fold Change in Significant DA microbes WATER vs FISH.png", plot = Water_Fish_DAplot1, path = "./Intermediate_R_objects/Figs", width = 12, height = 8, device='png', dpi=600)
```

Here, by plotting log2fold change in ASVs for both water and fish by season-region comparison, and filtering to only include fish which share >4 ASVs for a given region/season comparison, we can see something interesting: M. dorsalis and P. laticlavius (2 herbivores, but from different families) have an almost perfect 1-to-1 relationship with these shared DA taxa (r^2 ranging from 0.84-0.94, p < 0.001), suggesting that this portion of their microbiome is likely shifting due to changes in the surrounding seawater's microbiome. These shared DA taxa make up around 25-30% of these fishes overall DA taxa (M. dor DA taxa total across all 4 region/season comparisons = 42 (so 13/42 = 31%), and P. lat total DA taxa = 73 (so 20/73 = ~27%)). A. xanthopterus is a nice comparison, which has quite a few shared DA taxa with water, but the amount they change differs, such that the relationship between fish and water samples is not significant (r^2 = 0.3, p = 0.392)
