...
Script N. 3 - Differential abundance analyses - water & merged skin & water
...
#unrarefied dataset
ROHR_08_obj_phylo_root <- readRDS("./Intermediate_R_objects/ROHR_08_with_phylogenetic_tree_rooted.rds")
```
...
saveRDS(ROHR06_new, "./Intermediate_R_objects/ROHR_06_obj_newMetadata_cleaned.rds")
...
saveRDS(ROHR08_new, "./Intermediate_R_objects/ROHR_08_obj_newMetadata_cleaned.rds")

#read if not starting from top
ROHR08_new = readRDS("./Intermediate_R_objects/ROHR_08_obj_newMetadata_cleaned.rds")
```
...
ROHR_merged #7815 taxa & 360 samples
```

```{r save phyloseq merged datasets}
#Save phyloseq object with filtered taxa - fish
saveRDS(ROHR_merged, "./Intermediate_R_objects/ROHR_merged_skin_and_water_NO_TREE.rds")
```

```{r read merged water and fish datasets}
#read if not starting from top
ROHR_merged = readRDS("./Intermediate_R_objects/ROHR_merged_skin_and_water_NO_TREE.rds")
```
...
*LLL NOTE*: (2-16-26) updating this to keep the ASVs and the new taxa names together as a key, which can then be used downstream to compare sequences (& run BLAST, among other things)

```{r change OTU IDs from sequences to taxonomy}
# Extract the tax_table
tax_table_data_W <- tax_table(ROHR_08_filt)

# Store original taxa names (sequences / OTU IDs)
original_taxa_names_W <- taxa_names(ROHR_08_filt)

# Create a vector of new taxa names
new_taxa_names_W <- apply(tax_table_data_W, 1, function(row) {
  # Extract the phylum
  phylum <- row["Phylum"]
...
new_taxa_names_W <- make.unique(new_taxa_names_W)

# Create key table BEFORE overwriting taxa_names (to be able to link taxa back to sequences)
taxa_key_table_W <- data.frame(
  original_taxa_name = original_taxa_names_W,
  new_taxa_name = new_taxa_names_W,
  stringsAsFactors = FALSE
)

# Assign the new taxa names to the taxa_names
taxa_names(ROHR_08_filt) <- new_taxa_names_W
...
We also have a key table we can refer back to for when we want to recover the original ASVs linked to each of these taxa, which we'll want to save as well.

```{r save key table as csv}
write_csv(taxa_key_table_W, file = "./Intermediate_R_objects/ROHR_08_obj_newTaxanames_KEY_WATER.csv")
```

```{r save modified dataset with new taxa names}
#Save phyloseq object with filtered taxa
saveRDS(ROHR_08_filt, "./Intermediate_R_objects/ROHR_08_obj_10percent_newTaxanames.rds")
```

### START Here if already filtered data
```{r read modified water dataset w/ new taxa names}
#read if not starting from top
ROHR_08_filt = readRDS("./Intermediate_R_objects/ROHR_08_obj_10percent_newTaxanames.rds")
```
...
pheatmap(assay(ntd)[select,], cluster_rows=FALSE, show_rownames=TRUE, #can switch to rownames = FALSE for cleaner plot
...
For example: contrast=c("condition","treated","untreated") = log2(treated / untreated)
--> this means that the first contrast group (e.g. Las_Perlas_non_upwelling) is treated as the "treatment" group and the second one (e.g. Las_Perlas_upwelling) is treated as the "control" group. 
--> thus, a LFC > 0 (positive) means taxa are ENRICHED in the *first* group (or less abundant in the second, depending on your point of view)
--> on the flip side, a LFC < 0 (negative) means taxa are ENRICHED in the *second* group relative to the first group
...
non-upwelling (wet) = reference, so positive (up) taxa are enriched in the wet (non-upwelling) season, whiel LFC < 0 (down) are enriched during the upwelling (dry) season
...
```{r ALT filter only significant DA taxa per group}
# Filter for significant padj values (< 0.05) per contrast
top_resultsWater <- all_results %>%
  filter(!is.na(padj), padj < 0.05) %>%  # Remove NA values and filter for significant padj
  group_by(contrast) %>%
  ungroup() #1455 obs (too much to plot, but helpful for our comparisons downstream)
```

```{r extract DA sequences (and their tax assignments as of now) for all water samples}
WaterDA_seqs <- taxa_key_table_W %>%
  filter(new_taxa_name %in% top_resultsWater$Species) %>%  #select only DA taxa
  left_join(top_resultsWater %>%
      select(Species, contrast, log2FoldChange), #add back in taxonomy, seasonal comparison, and direction of change
    by = c("new_taxa_name" = "Species")
    ) %>%
  mutate(host_species = "Water") %>%  #add column to specify which host species this was found in
  select(host_species, contrast, new_taxa_name, everything()) #reorder columns to be more readable
...
```{r rename problem taxa}
top_resultsWater2 <- top_resultsWater %>%
  mutate(
    Species = ifelse(
...
# IN PROGRESS 2-16-26 - LLL: still need transfer this code to the fish dataset

```{r extract correct sequence inf}
#dereplicate taxa names
top_resultsWater_collapsed <- top_resultsWater2 %>%
  
  # Merge contrast + log2FC
  mutate(
    contrast_log2FC = paste(contrast, log2FoldChange)
  ) %>%
  
  # Count duplicates per Species
  group_by(Species) %>%
  mutate(n_occur = n()) %>%
  
  # Collapse duplicate Species rows
  summarise(
...
#check this worked
top_resultsWater_collapsed #732 obs

# First pass: exact match
first_pass <- taxa_key_table_W %>%
  left_join(
    top_resultsWater_collapsed,
    by = c("new_taxa_name" = "Species")
  ) #1298 obs

#flag unmatched
matched_species <- first_pass %>%
  filter(!is.na(contrast_log2FC)) %>%
  pull(new_taxa_name) #732

unmatched_species <- setdiff(
  top_resultsWater_collapsed$Species,
  matched_species
) #0 YAY!

#second pass: search for matches after ';', since some don't have phylum appended (e.g. Marinimicrobia)
#second_pass <- taxa_key_table_W %>%
#  filter(
#    str_replace(new_taxa_name, "^[^;]+;\\s*", "") %in% unmatched_species
#  ) %>%
#  left_join(
#    top_resultsWater_collapsed,
#    by = c("new_taxa_name" = "Species")
...
#merge both of these
WaterDA_seqs <- bind_rows(
  first_pass %>% filter(!is.na(contrast_log2FC)),
  second_pass
) %>%
  mutate(host_species = "Water") %>%
  select(
    host_species,
    new_taxa_name,
    n_occur,
...
#check we're recovering all significant DA taxa (should be 1455 for water based on the 'top_resultsWater' df)
total_n_occur <- WaterDA_seqs %>%
  summarise(total_n = sum(n_occur, na.rm = TRUE)) %>%
  pull(total_n)

total_n_occur #1455 - finally!!!

#troubleshoot this (if needed)
#species_top <- unique(top_resultsWater2$Species)
#species_final <- unique(WaterDA_seqs$new_taxa_name)

#missing_species <- setdiff(species_top, species_final)

#length(missing_species)
#missing_species
```

Beautiful! We now have a data frame with all the unique DA ASVs for the water dataset, as well as some extra info on how many times they are differentially abundant, and a string (separated by ";") of contrasts + log2FoldChange values that show the region-season comparisons for which these taxa were differentially abundant, as well as the direction of change.

```{r save DA taxa list}
write_csv(WaterDA_seqs, file = "./Intermediate_R_objects/WaterDAsequences_DESeq2_20260216.csv")
```
...
# Extract the tax_table
tax_table_data_W <- tax_table(ROHR_08_filt_Perlas)
...
# Assign the new taxa names to the taxa_names
taxa_names(ROHR_08_filt_Perlas) <- new_taxa_names_W
...
```{r save modified dataset with new taxa names}
#Save phyloseq object with filtered taxa
saveRDS(ROHR_08_filt_Perlas, "./Intermediate_R_objects/ROHR_08_PERLAS_20percent_newTaxanames.rds")

#read if not starting from top
ROHR_08_filt_Perlas = readRDS("./Intermediate_R_objects/ROHR_08_PERLAS_20percent_newTaxanames.rds")
```
...
mas_1 <- Maaslin2(
  ...
  output = "./Intermediate_R_objects/Maaslin_default_output_Water",
  min_abundance = 0.0,
  min_prevalence = 0.0,
...
results_folder <- "./Intermediate_R_objects/Maaslin_default_output_Water"
...
```{r attempt to change OTU IDs}
# Extract the tax_table
tax_table_data_W <- tax_table(ROHR_merged_filt)
...
# Assign the new taxa names to the taxa_names
taxa_names(ROHR_merged_filt) <- new_taxa_names_W
...
```{r save modified dataset with new taxa names}
#Save phyloseq object with filtered taxa
saveRDS(ROHR_merged_filt, "./Intermediate_R_objects/ROHR_merged_10percent_newTaxanames.rds")

#read if not starting from top
ROHR_merged_filt = readRDS("./Intermediate_R_objects/ROHR_merged_10percent_newTaxanames.rds")
```
...
mas_2 <- Maaslin2(
  ...
  output = "./Intermediate_R_objects/Maaslin_default_output2",
  min_abundance = 0.0,
  min_prevalence = 0.0,
  normalization = "TSS",
  transform = "LOG",
